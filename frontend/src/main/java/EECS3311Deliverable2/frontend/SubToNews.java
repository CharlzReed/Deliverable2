...
public class SubToNews {
		...
		searchButton.addActionListener(e -> {
            ...
            for (Item item:searched){
                if (item.getItemType()==ItemType.SUBSCRIPTION&&!currentnews.contains(item)){
                    booklist.addElement(item.name + " " + "(" + item.itemType + ") " + "(" + String.format("%.2f", item.cost) + ")");
    ...
    private void loadData(DefaultListModel<String>items123,HashMap<String,Item>list,DefaultListModel<String>itemsowned,HashMap<String,Item>ownedlist){
    ...
    private void additemtocart(JPanel j,HashMap<String, Item> item, String key){
        ...
        if (canrent==true){
            ...
            JOptionPane.showMessageDialog(j, "Reason: " + grabDenyReason(this.user.rentalDenied), "Item Cannot Be Added", JOptionPane.OK_OPTION);
        }
