...
public class RentPhysItem {
	...
	private JPanel show(JPanel j) {
		...
		// Storing description of items to show to the user, along with the list of
		// items
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (Item item : results) {
					if (item.isSubscription() == false) {
						itemList.addElement(item.name + " " + "(" + item.itemType + ") " + "("
	...
	private void addItemToCart(JPanel j, HashMap<String, Item> item, String key) {
		...
		if (canRent == true) {
			...
			JOptionPane.showMessageDialog(j, "Reason: " + grabDenyReason(this.currentUser.rentalDenied),
